...
while ${run}; do
  ...
  python3 conflate2rss/conflate2rss.py -n /jsons/current/changes.json -i /jsons/inspected/changes.json -r /rss/rss.xml -w /rss/raw_rss.json -p /jsons/history/ --rssurl http://46.101.158.129:8080/rss/rss.xml --rssauthor Janjko --rsslanguage en
  sleep ${PERIOD:-24h}
done